namespace Csag.WorkloadIdentity.UnitTests
{
    ...
    /// <summary>
    /// Unit tests for the <see cref="ResourceTokenProvider"/> class.
    /// </summary>
    public class ResourceTokenProviderTests
    {
        /// <summary>
        /// The resource the provider under test holds the token for.
        /// </summary>
        private const TokenScope Scope = TokenScope.AzureSql;
        ...
        /// <summary>
        /// Verifies that the first call obtains a token and returns it with its real expiry.
        /// </summary>
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_NoHeldToken_When_GetAccessTokenAsync_Then_ExchangesAndReturnsTokenWithExpiry()
        {
            // Arrange
            var expiresOn = StartTime.AddHours(1);
            this.SetupExchange(new AccessToken(FirstToken, expiresOn));
            ...
            // Assert
            result.Token.ShouldBe(FirstToken);
        ...
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_HeldTokenOutsideRefreshWindow_When_GetAccessTokenAsync_Then_ReturnsHeldTokenWithoutExchange()
        {
            // Arrange
        ...
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_HeldTokenOneTickOutsideRefreshWindow_When_GetAccessTokenAsync_Then_ReturnsHeldToken()
        {
            // Arrange
            var expiresOn = StartTime.AddHours(1);
        ...
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_HeldTokenEnteringRefreshWindow_When_GetAccessTokenAsync_Then_ExchangesNewToken()
        {
            // Arrange
            var expiresOn = StartTime.AddHours(1);
            this.SetupExchange(new AccessToken(FirstToken, expiresOn), new AccessToken(SecondToken, expiresOn.AddHours(1)));
            ...
            // Assert
            result.Token.ShouldBe(SecondToken);
        ...
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_ExchangedTokenAlreadyInsideRefreshWindow_When_GetAccessTokenAsync_Then_UsesItAndRefreshesOnNextCall()
        {
            // Arrange
            ...
            // Assert
            first.Token.ShouldBe(FirstToken);
            second.Token.ShouldBe(SecondToken);
            await this.ShouldHaveExchangedAsync(2);
        }
        ...
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_FiftyConcurrentCallersAndNoHeldToken_When_GetAccessTokenAsync_Then_ExchangesExactlyOnce()
        {
            // Arrange
            var exchange = new TaskCompletionSource<AccessToken>(TaskCreationOptions.RunContinuationsAsynchronously);
            this.exchanger.ExchangeAsync(Scope, Arg.Any<CancellationToken>()).Returns(exchange.Task);
            using var provider = this.CreateProvider();

            // Act
            var calls = Enumerable.Range(0, 50)
                .Select(_ => Task.Run(() => provider.GetAccessTokenAsync(CancellationToken.None)))
                .ToArray();
            ...
            // Assert
            results.ShouldAllBe(token => token.Token == FirstToken);
            await this.ShouldHaveExchangedAsync(1);
        }
        ...
        /// <returns>A task that represents the asynchronous operation.</returns>
        [Fact]
        public async Task Given_FailedExchange_When_GetAccessTokenAsyncAgain_Then_RetriesExchange()
        {
            // Arrange
            this.exchanger.ExchangeAsync(Scope, Arg.Any<CancellationToken>())
                .Returns(
            ...
            // Act
            var failure = await Should.ThrowAsync<InvalidOperationException>(() => provider.GetAccessTokenAsync(CancellationToken.None));
            var result = await provider.GetAccessTokenAsync(CancellationToken.None);
        ...
        public async Task Given_HeldTokenOutsideRefreshWindow_When_RefreshAsync_Then_ExchangesNewTokenAndReturnsItsExpiry()
        {
            ...
            // Act
            var expiresOn = await provider.RefreshAsync(CancellationToken.None);
            var result = await provider.GetAccessTokenAsync(CancellationToken.None);
        ...
        public async Task Given_ExchangeInFlight_When_RefreshAsync_Then_SharesThatExchange()
        {
            ...
            // Act
            var call = Task.Run(() => provider.GetAccessTokenAsync(CancellationToken.None));
            await Task.Delay(TimeSpan.FromMilliseconds(200));
            var refresh = Task.Run(() => provider.RefreshAsync(CancellationToken.None));
            await Task.Delay(TimeSpan.FromMilliseconds(200));
            exchange.SetResult(new AccessToken(FirstToken, expiresOn));

            // Assert
            (await call).Token.ShouldBe(FirstToken);
        ...
        {
            await this.exchanger.Received(times).ExchangeAsync(Scope, Arg.Any<CancellationToken>());
        }
        ...
        private ResourceTokenProvider CreateProvider()
        {
            return new ResourceTokenProvider(Scope, this.exchanger, RefreshAheadWindow, this.timeProvider, NullLogger<ResourceTokenProvider>.Instance);
        }
    }
}
